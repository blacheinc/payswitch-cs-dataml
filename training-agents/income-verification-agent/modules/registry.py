...
def _should_promote(
    ml_client, registry_name: str, new_metrics: dict, primary_metric: str = "weighted_f1",
) -> tuple[bool, dict]:
...
def log_and_register_model(
    ...
    try:
        ...
        if not should_register:
            logger.warning("Model NOT promoted — new F1 %.4f <= champion F1 %.4f (v%s)",
                           metrics.get("weighted_f1", 0), champion_info.get("champion_value", 0),
