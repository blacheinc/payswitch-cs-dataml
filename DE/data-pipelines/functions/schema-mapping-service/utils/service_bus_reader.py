"""
Service Bus Reader
Utility for reading messages from Service Bus topics (primarily for testing/debugging)
"""
...
try:
    from .service_bus_writer import ServiceBusClientError
except ImportError:
    from utils.service_bus_writer import ServiceBusClientError
...
class ServiceBusReader:
    """
    Service Bus reader for testing and debugging
    ...
    def __init__(
        ...
        self._key_vault_reader: Optional[KeyVaultReader] = None
        self._receiver: Optional[ServiceBusReceiver] = None
    
    def _get_key_vault_reader(self) -> KeyVaultReader:
        ...
        if self._key_vault_reader is None:
            ...
            self._key_vault_reader = KeyVaultReader(key_vault_url=self.key_vault_url)
            ...
            with client:
                ...
                with receiver:
                    ...
                    for message in received_messages:
                        try:
                            # Handle message body - try different methods based on SDK version
                            body = None
                            ...
                            # Dead-letter the message if we can't process it
                            receiver.dead_letter_message(message)
            
            logger.info(f"Read {len(messages)} messages from {topic_name}/{subscription_name}")
            return messages
            
        except Exception as e:
            logger.error(f"Error reading messages: {str(e)}")
            raise ServiceBusClientError(f"Failed to read messages: {str(e)}") from e
    ...
    def peek_messages(
        ...
        try:
            ...
            raise ServiceBusClientError(f"Failed to peek messages: {str(e)}") from e
    
    def get_receiver(
        self,
        topic_name: str,
        subscription_name: str,
        max_lock_duration: int = 120  # 2 minutes default
    ) -> ServiceBusReceiver:
        """
        Get or create a Service Bus receiver for a subscription
    ...
    def receive_next_message(
        ...
        """
        try:
            receiver = self.get_receiver(topic_name, subscription_name)
            ...
            raise ServiceBusClientError(f"Failed to receive message: {str(e)}") from e
    ...
    def complete_message(self, receiver: ServiceBusReceiver, message: ServiceBusMessage) -> None:
        ...
        try:
            ...
            raise ServiceBusClientError(f"Failed to complete message: {str(e)}") from e
    ...
    def abandon_message(self, receiver: ServiceBusReceiver, message: ServiceBusMessage) -> None:
        ...
        try:
            ...
            raise ServiceBusClientError(f"Failed to abandon message: {str(e)}") from e
    ...
    def dead_letter_message(
        ...
        """
        try:
            receiver.dead_letter_message(message, reason=reason)
            logger.info(f"Dead-lettered message: {message.message_id}, reason: {reason}")
        except Exception as e:
            logger.error(f"Error dead-lettering message: {str(e)}")
            raise ServiceBusClientError(f"Failed to dead-letter message: {str(e)}") from e
