"""
Script to send a test message WITHOUT run_id to Service Bus (Fail Path Test)
This is for Phase 2 testing - verifying messages without run_id are rejected
"""
...
def send_test_message_without_run_id(
    key_vault_url: str,
    ...
    """
    Send a test message WITHOUT run_id to Service Bus (Fail Path Test)
    ...
    # Create message payload WITHOUT run_id (intentionally missing)
    message_data = {
        # "run_id": intentionally omitted for fail path test
    ...
    print("="*60)
    print("Sending Test Message WITHOUT run_id (Fail Path Test)")
    print("="*60)
    print(f"Training Upload ID: {training_upload_id}")
    print(f"Bank ID: {bank_id}")
    print(f"Bronze Blob Path: {bronze_blob_path}")
    print()
    print("[WARNING] This message is intentionally missing 'run_id' field")
    print("          in both message body AND application_properties")
    print("Expected behavior:")
    print("  - Service Bus filter should REJECT message (run_id IS NOT NULL fails)")
    print("  - Message should NOT appear in subscription")
    print("  - This tests the Service Bus filter, not code validation")
    print()
    ...
    try:
        ...
        # Set custom properties WITHOUT run_id (to test Service Bus filter rejection)
        message.application_properties = {
            "subscription": "start-transformation",
            "source": "test-script-without-run-id",
            # "run_id": intentionally omitted to test Service Bus filter
            "training_upload_id": training_upload_id  # Still include for filter
        }
        ...
        print()
        print("[NOTE] run_id is intentionally MISSING from application_properties")
        print("       This will cause Service Bus filter to reject the message")
        print()
        ...
        print("Expected behavior:")
        print("  - Service Bus filter should REJECT message (run_id IS NOT NULL fails)")
        print("  - Message should NOT appear in 'start-transformation' subscription")
        print("  - Message will remain in topic but filtered out by subscription")
        print("  - This tests that the Service Bus filter is working correctly")
        print()
        print("To verify filter is working:")
        print("  - Check subscription message count (should NOT increase)")
        print("  - Message should NOT be received by pipeline script")
...
if __name__ == "__main__":
    ...
    try:
        send_test_message_without_run_id(
