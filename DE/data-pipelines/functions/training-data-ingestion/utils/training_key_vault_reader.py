"""
Training Data Ingestion - Key Vault Reader
...
class TrainingKeyVaultError(Exception):
...
class TrainingKeyVaultReader:
    ...
    def __init__(
        ...
        if not KEY_VAULT_AVAILABLE:
            raise TrainingKeyVaultError(
                "Azure Key Vault SDK not available. Install with: pip install azure-keyvault-secrets"
            )
        
        if not key_vault_url:
            raise TrainingKeyVaultError("key_vault_url is required and cannot be empty")
    ...
    def get_secret(
        ...
        Raises:
            TrainingKeyVaultError: If secret cannot be retrieved
        """
        if not secret_name:
            raise TrainingKeyVaultError("secret_name is required and cannot be empty")
        ...
        try:
            ...
            if value is None:
                raise TrainingKeyVaultError(f"Secret '{secret_name}' exists but has no value")
            ...
            if (not self._credential_excludes_env and 
                ...
                try:
                    ...
                    raise TrainingKeyVaultError(error_msg) from retry_e
            else:
                ...
                logger.error(error_msg)
                raise TrainingKeyVaultError(error_msg) from e
        except Exception as e:
    ...
    def get_secrets(
        ...
        Raises:
            TrainingKeyVaultError: If any secret cannot be retrieved
        """
        if not secret_names:
            return {}
    ...
    def __enter__(self) -> 'TrainingKeyVaultReader':
